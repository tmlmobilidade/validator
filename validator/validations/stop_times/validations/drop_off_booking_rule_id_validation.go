...
func DropOffBookingRuleIdValidation(stopTime *types.StopTime, row int, gtfs *types.Gtfs, rules *types.StopTimesRules) {
	ctx := lib.NewValidationContext("drop_off_booking_rule_id", "stop_times.txt", "drop_off_booking_rule_id_validation", "drop_off_booking_rule_id_references_booking_rules_or_empty", row, services.AppMessageService)
