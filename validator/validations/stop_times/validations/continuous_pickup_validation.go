...
func ContinuousPickupValidation(stopTime *types.StopTime, row int, rules *types.StopTimesRules) {
	ctx := lib.NewValidationContext("continuous_pickup", "stop_times.txt", "continuous_pickup_validation", "stop_times_continuous_pickup_valid_gtfs_enum", row, services.AppMessageService)
