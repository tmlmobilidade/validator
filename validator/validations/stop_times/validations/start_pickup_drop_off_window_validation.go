...
func StartPickupDropOffWindowValidation(stopTime *types.StopTime, row int, rules *types.StopTimesRules) {
	ctx := lib.NewValidationContext("start_pickup_drop_off_window", "stop_times.txt", "start_pickup_drop_off_window_validation", "start_pickup_drop_off_window_rule", row, services.AppMessageService)
