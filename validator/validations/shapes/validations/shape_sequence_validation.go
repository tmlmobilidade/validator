...
func ShapeSequenceValidation(shapes []types.Shape) {
	...
	for i, shape := range shapes {
		ctx := lib.NewValidationContext("shape_pt_sequence", "shapes.txt", "shape_pt_sequence_validation", "shape_pt_sequence_rule", i, services.AppMessageService)
	...
	for _, shapeGroup := range shapeGroups {
		...
		for i, shape := range shapeGroup {
			if i > 0 {
				ctx := lib.NewValidationContext("shape_pt_sequence", "shapes.txt", "shape_pt_sequence_validation", "shape_pt_sequence_rule", shape.row, services.AppMessageService)
				...
				if shape.dist >= 0 && shapeGroup[i-1].dist >= 0 {
					if shape.dist < shapeGroup[i-1].dist {
						ctxDist := lib.NewValidationContext("shape_dist_traveled", "shapes.txt", "shape_dist_traveled_validation", "shape_dist_traveled_rule", shape.row, services.AppMessageService)
