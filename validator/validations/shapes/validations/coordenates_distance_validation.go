...
func ShapeCoordinatesDistanceValidation(shapes []types.Shape) {
	...
	for _, shapeGroup := range shapeGroups {
		...
		for i := 1; i < len(shapeGroup); i++ {
			...
			closeEnough, _ := shapes_coordinates.ShapePointIsCloseToBeforeShapePoint(prevShape, currentShape)
			if closeEnough {
				continue
			}
			...
			if i+1 < len(shapeGroup) {
				...
				prevAndNextAreClose, _ := shapes_coordinates.ShapePointIsCloseToBeforeShapePoint(prevShape, nextShape)
