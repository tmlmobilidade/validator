...
func ContinuousPickupValidation(route *types.Route, row int, gtfs *types.Gtfs, rules *types.RoutesRules, routesWithWindows map[string]bool) {
	ctx := lib.NewValidationContext("continuous_pickup", "routes.txt", "continuous_pickup_validation", "continuous_pickup_valid_gtfs_enum", row, services.AppMessageService)
