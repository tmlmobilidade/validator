...
func DefaultLangValidation(severity *types.Severity, feedInfo *types.FeedInfo, row int) {
	ctx := lib.NewValidationContext("default_lang", "feed_info.txt", "default_lang_validation", "default_lang_matches_feed_lang_when_present", row, services.AppMessageService)
	if severity != nil {
