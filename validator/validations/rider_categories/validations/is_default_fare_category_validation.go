...
func IsDefaultFareCategoryValidation(riderCategory *types.RiderCategory, row int, rules *types.RiderCategoriesRules) {
	ctx := lib.NewValidationContext("is_default_fare_category", "rider_categories.txt", "is_default_fare_category_validation", "validate_is_default_fare_category", row, services.AppMessageService)
