...
func EmissionValidation(vehicle *types.Vehicle, row int, rules *types.VehiclesRules) {
	ctx := lib.NewValidationContext("emission", "vehicles.txt", "emission_validation", "validate_emission", row, services.AppMessageService)
	...
	validOptions := []int{0, 1, 2, 3, 4, 5, 6, 7, 0}
