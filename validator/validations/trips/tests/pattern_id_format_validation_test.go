...
func TestAllPatternIdFormatValidationTestCases(t *testing.T) {
	...
	t.Run("Valid Pattern ID For CA Format", func(t *testing.T) {
		services.AppMessageService.Clear()
		trip := &types.Trip{PatternId: lib.Ptr("1001_0_ABC")}
		gtfs, cleanup, err := test_helpers.MockGtfs{IdMapData: types.GtfsIdMap{"trips": {"1001_0_ABC": []int{1}}}}.ToGtfsWithDB()
		if err != nil {
			t.Fatalf("failed to create mock gtfs: %v", err)
		}
		defer cleanup()
		validations.PatternIdFormatValidation(trip, 1, gtfs, &types.TripsRules{
			PatternIdFormat: types.RuleConfig{
				...
				Options:  lib.Ptr([]string{"XXXX_X_XXX"}),
			},
		})
		test_helpers.AssertMessageCount(t, services.AppMessageService, 0, "Valid Pattern ID For CA Format", types.SEVERITY_ERROR)
	})

	t.Run("Invalid Pattern ID For CA Format", func(t *testing.T) {
		services.AppMessageService.Clear()
		trip := &types.Trip{PatternId: lib.Ptr("1001_0_6")}
		...
		test_helpers.AssertMessageCount(t, services.AppMessageService, 1, "Invalid Pattern ID For CA Format", types.SEVERITY_ERROR)
	})
}
