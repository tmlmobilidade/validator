...
func WheelchairAccessibleValidation(trip *types.Trip, row int, gtfs *types.Gtfs, rules *types.TripsRules) {
	ctx := lib.NewValidationContext("wheelchair_accessible", "trips.txt", "wheelchair_accessible_validation", "wheelchair_accessible_rule", row, services.AppMessageService)
