...
func PatternIdGroupValidation(tripsGroupedByPattern types.TripGroupedByPattern, gtfs *types.Gtfs, rules *types.TripsRules) {
	...
	for patternId, group := range tripsGroupedByPattern {
		...
		for _, trip := range group.Trips {
			ctx := lib.NewValidationContext("pattern_id", "trips.txt", "pattern_id_validation", "validate_pattern_id_group", trip.Row, services.AppMessageService)
		...
		if len(group.Hash) > 1 {
			...
			ctx := lib.NewValidationContext("pattern_id", "trips.txt", "pattern_id_validation", "validate_pattern_id_group", row, services.AppMessageService)
			if rules != nil && rules.PatternIdGroup.Severity != "" {
				ctx.WithSeverity(rules.PatternIdGroup.Severity)
			}
			if ctx.ShouldSkip() {
