...
func PatternIdGroupValidation(tripsGroupedByPattern types.TripGroupedByPattern, gtfs *types.Gtfs, rules *types.TripsRules) {
	...
	for patternId, group := range tripsGroupedByPattern {
		...
		for _, trip := range group.Trips {
			ctx := lib.NewValidationContext("pattern_id", "trips.txt", "pattern_id_validation", "pattern_id_trip_has_required_fields_for_grouping", trip.Row, services.AppMessageService)
		...
		if len(group.Hash) > 1 {
			...
			ctx := lib.NewValidationContext("pattern_id", "trips.txt", "pattern_id_validation", "pattern_id_single_trip_signature_per_pattern", row, services.AppMessageService)
			if rules != nil && rules.PatternIdGroup.Severity != "" {
				ctx.WithSeverity(rules.PatternIdGroup.Severity)
			}
			if ctx.ShouldSkip() {
