...
func AgencyIdValidation(fareAttribute *types.FareAttribute, row int, gtfs *types.Gtfs, rules *types.FareAttributesRules) {
	ctx := lib.NewValidationContext("agency_id", "fare_attributes.txt", "agency_id_validation", "fare_attributes_agency_id_references_agency_table", row, services.AppMessageService)
