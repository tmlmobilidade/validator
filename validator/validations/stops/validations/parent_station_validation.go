...
func ParentStationValidation(stop *types.Stop, row int, gtfs types.Gtfs, rules *types.StopsRules) {
	ctx := lib.NewValidationContext("parent_station", "stops.txt", "parent_station_validation", "parent_station_id_valid_for_stop_hierarchy", row, services.AppMessageService)
