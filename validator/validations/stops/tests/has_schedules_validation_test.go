...
func TestHasSchedulesValidation_InvalidInput_SeverityError(t *testing.T) {
	services.AppMessageService.Clear()
	val := 9999
	stop := &types.Stop{HasSchedules: &val}
	severity := types.SEVERITY_ERROR
	validations.HasSchedulesValidation(stop, 5, &types.StopsRules{HasSchedules: types.RuleConfig{Severity: severity}})
	assertion := lib.AssertionMessage{
		Expected: 1,
		Actual:   services.AppMessageService.GetSummary().TotalErrors,
		Message:  "Invalid has_Schedules should error",
	}
	if assert := lib.Assert(assertion); assert != "" {
		t.Error(assert)
	}
...
func TestHasSchedulesValidation_ValidInput_WithOptions(t *testing.T) {
	services.AppMessageService.Clear()
	val := 1
	stop := &types.Stop{HasSchedules: &val}
	severity := types.SEVERITY_ERROR
	validations.HasSchedulesValidation(stop, 7, &types.StopsRules{HasSchedules: types.RuleConfig{Severity: severity, Options: &[]string{"1", "2"}}})

	assertion := lib.AssertionMessage{
		Expected: 0,
		Actual:   services.AppMessageService.GetSummary().TotalErrors,
		Message:  "Valid has_Schedules with severity ERROR should not error",
	}
	if assert := lib.Assert(assertion); assert != "" {
		t.Error(assert)
	}
...
func TestHasSchedulesValidation_InValidInput_WithOptions(t *testing.T) {
	services.AppMessageService.Clear()
	val := 5
	stop := &types.Stop{HasSchedules: &val}
	severity := types.SEVERITY_ERROR
	...
	if services.AppMessageService.GetSummary().TotalErrors != 1 {
		t.Error("Valid has_Schedules with severity ERROR should error")
	}
	assertion := lib.AssertionMessage{
		Expected: 1,
		Actual:   services.AppMessageService.GetSummary().TotalErrors,
		Message:  "Valid has_Schedules with severity ERROR should error",
	}
	if assert := lib.Assert(assertion); assert != "" {
		t.Error(assert)
	}
